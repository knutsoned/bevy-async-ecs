...
impl AsyncSystem {
    ...
    pub async fn run(&self) {
        let id = *self.id;
        self.world.apply(move |world: &mut World| {
...
mod tests {
    ...
    fn normal_unregister() {
        ...
        assert_counter!(id, 1, app.world());
        assert!(matches!(err, Err(RegisteredSystemError::SystemIdNotRegistered(_))));
    }
    ...
    fn io() {
        ...
        AsyncComputeTaskPool::get()
            .spawn(async move {
                let increase_counter = async_world.register_io_system::<Entity, (), _>(
                    increase_counter
                ).await;
                let get_counter_value = async_world.register_io_system::<Entity, u8, _>(
                    get_counter_value
                ).await;
